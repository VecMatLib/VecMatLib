...
import io.github.hexagonnico.vecmat.vector.VectorDoubleOperations;

/**
 * Interface to be implemented by a matrix that can be represented as
 * a double matrix without casting.
 * 
 * @param <M> A double matrix of the same size as this one
...
public interface MatrixAsDouble<M extends MatrixDoubleOperations<M, V>, V extends VectorDoubleOperations<V>> {
	 ...
	 * 
	 * <p> The product of this matrix and a double scalar is a double matrix.
	 * 
	 * <p> Matrices are supposed to be immutable. This means that
	 * this method does not alter the object on which it is called,
	 * it returns a new matrix instead.
	...
	/**
	 * Converts this matrix to a double matrix without casting.
	 * 
	 * @return The double value of this matrix
	 */
	M asDouble();
}
